...
save_path = str(current_dir) + '/Generated-Images/ddpm_1000_steps_v4_nocfg'
...
def show_grids(images, feed_mod_value, epoch):
    ...
    for ax, im in zip(grid, images.cpu()):
        ax.axis('off')
...
for ep in epochs:
    load_dir = os.path.join(str(current_dir) + '/trained-model/', trained_ddpm_name_(ep))
    ...
    for feed_mod_value in gen_fm_values:
        for _ in range(n_samples_per_class):
            ...
            with torch.no_grad():
                t_fm_normalized = torch.tensor([[normalize_fm(feed_mod_value)]] * sample_batch_size).to(device) # Normalisiere und forme korrekt
                ...
                ema_sampled_images = diffusion.sample(ema_model, sample_batch_size, t_fm_normalized, cfg_scale=0)  # Change conditions here # 1 sample
